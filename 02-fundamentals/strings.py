...
helllo = r'Toto je dlouhý řetězec obsahující mnoho\n\
řádek textu, stejně jej zapisujete i v C.'
...
print(datum)
def text_na_identifikator(souslovi, jazyk):
    souslovi = unidecode(souslovi)
    identifikator = souslovi.lower().replace(" ", "_")
    if jazyk == "JS":
        identifikator = identifikator.split("_")
        identifikator = identifikator[0] + "".join([slovo.capitalize() for slovo in identifikator[1:]])
    return identifikator
text = input("Zadejte sousloví: ")
print(text_na_identifikator(text, "Python"))
print(text_na_identifikator(text, "JS"))
import random
def heslo(pocet):
    hesla = []
    for i in range(pocet):
        heslo = "".join([chr(random.randint(65, 90)) for i in range(3)])
        heslo += "".join([chr(random.randint(97, 122)) for i in range(3)])
        heslo += random.choice(["-", "+", "/", "*"])
        heslo += "".join([str(random.randint(0, 9)) for i in range(3)])
        hesla.append(heslo)
    return hesla
print(heslo(5))